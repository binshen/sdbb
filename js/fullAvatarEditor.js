function fullAvatarEditor() {
	...
	var file			= '/images/fullAvatarEditor.swf';		//flash文件的路径
	var	version			= "10.1.0";						//播放该flash所需的最低版本
	var	expressInstall	= '/images/expressInstall.swf';			//expressInstall.swf的路径
	...
	for(var i = 0; i < arguments.length; i++)
	{
		...
		if(typeof a == 'string')
		{
			if (a.substring(a.length - 'fullAvatarEditor.swf'.length) == '/sdbb/images/fullAvatarEditor.swf')
			{
				file = a;
			}
			else if (a.substring(a.length - 'expressInstall.swf'.length) == '/sdbb/images/expressInstall.swf')
			{
				expressInstall = a;
			}
			else
